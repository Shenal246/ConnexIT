...
function Networking() {
    ...
    useEffect(() => {
        const values = {
            query: "SELECT name,des,wlink,status,image_data,cnt,Networking FROM Vendor WHERE Networking='true' AND status=1 AND cnt=4;",
            key: "Cr6re8VRBm"
        };
    ...
    const handleCardClick = (vend) => {
        navigate(`/AU/Solutions/Networking/${vend.name}`, { state: { vend } });
    };
    return (
        <>
            <AUNavbar />
