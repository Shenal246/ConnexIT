...
function Endpoint() {
    ...
    useEffect(() => {
        const values = {
            query: "SELECT name,des,wlink,status,image_data,cnt,Endpoint_Security FROM Vendor WHERE Endpoint_Security='true' AND status=1 AND cnt=6;",
            key: "Cr6re8VRBm"
        };
    ...
    const handleCardClick = (vend) => {
        navigate(`/Brn/Solutions/Endpoin/${vend.name}`, { state: { vend } });
    };
    ...
    const { Brnep1, Brnep2
        } = t('Brnendpsec', { returnObjects: true });
            ...
            <div className="container">
                <div className='row '>
                    <div className="row text headingrow">
                        ...
                        <div className="col-4" data-aos="fade-up" data-aos-delay="100"><p id='topic'>{Brnep1}</p></div>
                        <div className="col-4" data-aos="fade-up" data-aos-delay="100"><hr /></div>
                    </div>
                </div>
