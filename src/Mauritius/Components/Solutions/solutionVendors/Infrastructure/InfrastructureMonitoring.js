...
function InfrastructureMonitoring() {
    ...
    useEffect(() => {
        const values = {
            query: "SELECT name,des,wlink,status,image_data,cnt,Data_Center_Infrastructure_and_Infrastructure_Monitoring FROM Vendor WHERE Data_Center_Infrastructure_and_Infrastructure_Monitoring='true' AND status=1 AND cnt=5;",
            key: "Cr6re8VRBm"
        };
